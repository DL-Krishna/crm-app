...
import { dataFilter, LearnerPageView, filterId } from '@/api/CommonData';
...
const LearnerPage = ({ handelOnSet }: { handelOnSet: (id: number, data: LeadeData[]) => void }) => {
    ...
    const [filterData, setFilterData] = useState<any>(LearnerPageView?.[0]?.value);
    ...
    const TopHeader = useMemo(() => LearnerPageView?.filter?.((item: any) => item?.value === filterData)?.[0]?.lable, [filterData])
    ...
    return (
        <div className="lg:w-full">
            ...
            {
                nav?.LeadStatus && <CreateLearner handelOnContactModel={handelOnContactModel} handelOnSave={handelOnSave} />
            }
        </div>
    )
}
