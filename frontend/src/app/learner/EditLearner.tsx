...
const EditLearner = ({ handelOnSet, learner }: { learner?: any, handelOnSet: (id: number, data: LeadeData[]) => void }) => {
    const dispatch = useAppDispatch()
    const [activeTab, setActiveTab] = useState<String>('Details');
    useMemo(() => {
        if (learner?.[0]?.id) {
            dispatch(getSingleLead(learner?.[0]?.id))
        }
    }, [learner?.[0]?.id])
    ...
    return (
        <div className="w-[fit-content] lg:w-full">
            <div className="m-5 rounded-lg sborder-4 border-zinc-100 shadow-lg">
                <div className="flex gap-3 items-center px-5 py-2 border-b border-b-neutral-400">
                    ...
                    <div className="flex gap-3 items-center">
                        ...
                        <h2 className="text-black text-lg font-semibold">{"Arjun"}</h2>
                    </div>
                ...
                <div className="py-2 px-14 flex flex-wrap justify-between">
                    <div className="min-w-48 mb-2">
                        ...
                        <span className="text-sky-600 text-sm font-semibold">{'Wesites'}</span>
                    </div>
                    <div className="min-w-48 mb-2">
                        <p className="text-black mb-1 text-base font-medium">Phone</p>
                        <span className="text-sky-600 text-sm font-semibold">+91 9876543210 </span>
                    </div>
                    <div className="min-w-48 mb-2">
                        <p className="text-black mb-1 text-base font-medium">Email</p>
                        <span className="text-sky-600 text-sm font-semibold">{'Digitallync@gmail.com'}</span>
                    </div>
                    <div className="min-w-48 mb-2">
            ...
            <div className="bg-white rounded-sm border-4 border-zinc-100 shadow-lg px-3 mx-5" >
                ...
                <div className='py-5'>
                    {activeTab === "Details" ? <LearnerDetail handelOnSet={handelOnSet} /> : activeTab === "Activity" ? <Activity /> : null}
                </div>
            </div>
        </div>
