...
import { dataFilter, DealListView, filterId } from '@/api/CommonData';
...
const LeadesPage = ({ handelOnSet }: { handelOnSet: (id: number, data: LeadeData[]) => void }) => {
    ...
    const TopHeader = useMemo(() => DealListView?.filter?.((item: any) => item?.value === filterData)?.[0]?.lable, [filterData])
    ...
    return (
        <div className="lg:w-full">
            ...
            {
                nav?.LeadStatus && <CreateLeade handelOnContactModel={handelOnContactModel} handelOnSave={handelOnSave} />
            }
        </div>
    )
}
