...
const CreateLearner = ({ handelOnCoursesModel, handelOnSave }: { handelOnSave: () => void, handelOnCoursesModel: () => void }) => {
    ...
    const vaidation = () => {
        ...
        if (!learner?.courseName?.trim()) {
            formValid = false
            newError["courseName"] = "Please enter course name"
        }
        if (!learner?.courseFee?.trim()) {
            formValid = false
            newError["courseFee"] = "Please enter course fee"
        }
    ...
    const handelOnSubmit = () => {
        if (vaidation()) {
            ...
            formData.append('name', learner?.courseName)
            formData.append('description', learner?.description)
            formData.append('courseFee', learner?.courseFee)
            ...
            dispatch(addCoursesData(formData)).unwrap()
                .then((res: any) => {
                    if (res?.status === 201) {
                        toast.success(res?.data?.message ? res?.data?.message : "Courses Create Successfully");
        ...
        <>
            <CustomModel headerImg={Contact} lable="Create Learner" onCancel={handelOnCancel} onSave={handelOnSubmit} isLoading={isLoading}>
                ...
                <div className="grid gap-6 my-8 md:grid-cols-2">
                    <CustomInput onChange={handelOnChang} lableValue="Course Name" value={learner?.courseName} error={error?.courseName} name="courseName" placeholder="Course Name" typeValue="text" />
                    <CustomInput onChange={handelOnChang} lableValue="Course Fee" value={learner?.courseFee} error={error?.courseFee} name="courseFee" placeholder="Course Fee" typeValue="text" />
